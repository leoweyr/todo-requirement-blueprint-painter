...
export class TimelineSlider extends Component<TimelineSliderProps, TimelineSliderState> {
    ...
    private _handleGlobalMouseUp: () => void = (): void => {
        if (this.state.isDragging) {
             ...
             const snapped = Math.round(currentPosition * 2) / 2;
    ...
    private _handleGlobalMouseMove: (event: globalThis.MouseEvent) => void = (event: globalThis.MouseEvent): void => {
        ...
        const pxPerUnit = 2 * this._TICK_SPACING;
        
        const deltaPos = deltaX / pxPerUnit;  // Dragging right increases the position.
    ...
    public constructor(props: TimelineSliderProps) {
        super(props);
        const maxIndex = Math.max(0, props.updateTimes.length - 1);
        ...
        let initialViewOffset = ((maxIndex * pxPerUnit) - this._VISIBLE_WINDOW_WIDTH / 2);
    ...
    private _renderCursor(): ReactNode {
        ...
        const cursorLeft = (currentPosition * pxPerUnit) - viewOffset + (this._TICK_SPACING / 2); 
        
        const style = this._getCursorStyle();
    ...
    private _updatePosition(delta: number, isDragging: boolean = false): void {
        const { updateTimes } = this.props;
        const maxPos = updateTimes.length - 1;
        const pxPerUnit = 2 * this._TICK_SPACING;
        
        this.setState((prevState: TimelineSliderState): TimelineSliderState => {
            let newPos = prevState.currentPosition + delta;
            ...
            const tickOffset = this._TICK_SPACING / 2;
            const cursorScreenX = (newPos * pxPerUnit) - prevState.viewOffset + tickOffset;
            
            let newViewOffset = prevState.viewOffset;
            let isLeftEdgeActive = false;
            let isRightEdgeActive = false;

            if (isDragging) {
                // If dragging, check edge constraints.
                const leftZone = this._EDGE_SCROLL_ZONE;
                const rightZone = this._VISIBLE_WINDOW_WIDTH - this._EDGE_SCROLL_ZONE;
                
                if (cursorScreenX < leftZone) {
                    const scrollAmount = (leftZone - cursorScreenX) * 0.2;  // Dampen.
                    newViewOffset -= scrollAmount;
                    isLeftEdgeActive = true;
                } else if (cursorScreenX > rightZone) {
                    const scrollAmount = (cursorScreenX - rightZone) * 0.2;
                    newViewOffset += scrollAmount;
            ...
            const snapped = Math.round(newPos * 2) / 2;
            const isTransition = snapped % 1 !== 0;
            const index = Math.floor(snapped);
    ...
    private _renderMajorLabel(index: number): ReactNode {
        const { updateTimes } = this.props;
        const { viewOffset } = this.state;
        const timeStr = updateTimes[index];
        
        const pxPerUnit = 2 * this._TICK_SPACING;
        const tickX = index * pxPerUnit; 
        const screenX = tickX - viewOffset + (this._TICK_SPACING / 2); 
        ...
        const distToRight = this._VISIBLE_WINDOW_WIDTH - screenX;
        const distToEdge = Math.min(distToLeft, distToRight);
        
        // Linear opacity: 0 at edge, 1 at FADE_WIDTH.
        let opacity = Math.max(0, Math.min(1, distToEdge / FADE_WIDTH));
    ...
    private _getVisualBgStyle(): CSSProperties {
        ...
        const stretchAmount = '24px';
        const isStretchingLeft = isDragging && isLeftEdgeActive;
        const isStretchingRight = isDragging && isRightEdgeActive;

        let transform = 'translateX(0)';
        let width = '100%';
        let boxShadow = '0 4px 20px rgba(0,0,0,0.15)';  // Default shadow.
